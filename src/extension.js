...
// =============================================================================
// [Configuration] Data Definitions (Colors, Formats, Autocomplete)
// =============================================================================
const colorMap = {
...
function activate(context) {
    // =========================================================================
    // Feature 1: Generate Missing Keys JSON
    // =========================================================================
    const generateMissingCommand = vscode.commands.registerCommand('minecraft-translator.generateMissingKeys', async () => {
        ...
        if (!fs.existsSync(originalPath)) {
            vscode.window.showErrorMessage("Could not find 'en_us.json' in the same directory.");
            return;
        }
        try {
            const currentJson = JSON.parse(editor.document.getText());
            const originalJson = JSON.parse(fs.readFileSync(originalPath, 'utf-8'));
            const missingJson = {};
            let missingCount = 0;
            for (const key in originalJson) {
                if (!(key in currentJson)) {
                    missingJson[key] = originalJson[key];
                    missingCount++;
                }
            }
            if (missingCount === 0) {
                vscode.window.showInformationMessage("No missing keys found! Perfect.");
                return;
            }
            const doc = await vscode.workspace.openTextDocument({
                content: JSON.stringify(missingJson, null, 4),
                language: 'json'
            });
            await vscode.window.showTextDocument(doc, vscode.ViewColumn.Beside);
            vscode.window.showInformationMessage(`Found ${missingCount} missing keys.`);
        }
        catch (e) {
            vscode.window.showErrorMessage("Error parsing JSON.");
        }
    });
    context.subscriptions.push(generateMissingCommand);
    // =========================================================================
    // Feature 2: Create New Language File (Empty Values)
    // =========================================================================
    const createLangFileCommand = vscode.commands.registerCommand('minecraft-translator.createLangFile', async () => {
        ...
        try {
            const originalJson = JSON.parse(editor.document.getText());
            const newJson = {};
            for (const key in originalJson) {
                newJson[key] = "";
            }
            ...
            vscode.window.showErrorMessage("JSON Parse Error: Invalid JSON format.");
        }
    });
    ...
    const scrollListener = vscode.window.onDidChangeTextEditorVisibleRanges(e => {
        if (!isScrollSyncEnabled || isSyncing) {
            return;
        }
    ...
    const validateJson = (doc) => {
        ...
        if (fileName === 'en_us.json') {
            // Do not validate en_us.json itself, but it triggers validation when opened
            return;
        }
        if (!fs.existsSync(originalPath)) {
            return;
        ...
        try {
            ...
            // B. Validate Original File (en_us): Check for missing keys
            for (const key in originalJson) {
                if (!(key in currentJson)) {
                    const escapedKey = key.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
                    const keyRegex = new RegExp(`"${escapedKey}"\\s*:`);
                    const match = keyRegex.exec(originalText);
                    if (match) {
                        const textBefore = originalText.substring(0, match.index);
    ...
    if (vscode.window.activeTextEditor) {
        validateJson(vscode.window.activeTextEditor.document);
    }
    // =========================================================================
    // Feature 5: Preview
    // =========================================================================
    const previewCommand = vscode.commands.registerCommand('minecraft-translator.showPreview', () => {
        ...
        update();
        const changeDoc = vscode.workspace.onDidChangeTextDocument(e => { if (e.document === editor.document) {
            update();
        } });
        const changeSel = vscode.window.onDidChangeTextEditorSelection(e => { if (e.textEditor === editor) {
            update();
        } });
    ...
    // =========================================================================
    // Feature 7: Highlighting (JSON Key Protected)
    // =========================================================================
    const decorations = new Map();
    function getDecoration(color, bold, italic, strike, underline, obfuscated) {
        ...
        if (!decorations.has(key)) {
            ...
            if (strike || underline) {
                options.textDecoration = `${strike ? 'line-through' : ''} ${underline ? 'underline' : ''}`.trim();
            }
            if (obfuscated) {
                options.border = '1px solid #888888';
                options.backgroundColor = 'rgba(136, 136, 136, 0.3)';
            }
            ...
            for (let i = 0; i < content.length; i++) {
                if (content[i] === '§' && i + 1 < content.length) {
                    if (currentRangeStart !== -1) {
                        ...
                        if (!decorationRanges.has(deco)) {
                            decorationRanges.set(deco, []);
                        }
                    ...
                    if (colorMap[code]) {
                        ...
                        isBold = isItalic = isStrike = isUnderline = isObfuscated = false;
                    }
                    else if (code === 'l') {
                        isBold = true;
                    }
...
function generatePreviewHtml(text) {
    ...
    lines.forEach(line => {
        ...
        const flushBuffer = () => {
            if (buffer.length > 0) {
                lineHtml += `<span style="${currentStyle}">${buffer}</span>`;
                buffer = '';
            }
        };
        ...
        for (let i = 0; i < line.length; i++) {
            if (line[i] === '§' && i + 1 < line.length) {
                ...
                if (colorMap[code]) {
                    ...
                    bold = italic = strike = underline = obfuscated = '';
                }
                else if (code === 'l') {
                    bold = formatMap['l'];
                }
                    ...
                    underline = formatMap['n'];
                }
                else if (code === 'o') {
                    italic = formatMap['o'];
                }
                else if (code === 'k') {
                    obfuscated = formatMap['k'];
                }
                else if (code === 'r') {
                    color = 'color: #FFFFFF;';
                    bold = italic = strike = underline = obfuscated = '';
...
<head>
    ...
    <style>
        ...
        button { cursor: pointer; padding: 8px 12px; background: #444; color: white; border: 1px solid #666; border-radius: 4px; font-size: 12px; }
...
<body class="bg-dark">
    <div class="controls">
        <button onclick="toggleTheme()">🌗 Toggle Theme (B/W)</button>
    </div>
    <div id="content">
        ${htmlBody}
    </div>
